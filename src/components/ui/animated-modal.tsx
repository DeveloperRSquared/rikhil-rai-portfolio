...
export const ModalTrigger = ({ children, className }: { children: ReactNode; className?: string }) => {
  ...
  return (
    <div
      className={cn('py-2 rounded-md text-black dark:text-white text-center relative', className)}
    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className="fixed [perspective:800px] [transform-style:preserve-3d] inset-0 w-full  h-full flex items-center justify-center z-[9999]"
        >
          <Overlay />
          ...
          <motion.div
            ref={modalRef}
            className={cn(
              'min-h-[80%] max-h-[90%] md:max-w-[40%] bg-white dark:bg-neutral-950 border border-transparent dark:border-neutral-800 md:rounded-2xl relative z-50 flex flex-col flex-1',
              className,
            )}
            initial={{
              opacity: 0,
...
const CloseIcon = () => {
  const { setOpen } = useModal();
  return (
    <button type="button" onClick={() => setOpen(false)} className="absolute top-4 right-4 group" aria-label="Close">
      <svg
