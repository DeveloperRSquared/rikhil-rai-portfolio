...
const CarouselSlider = ({ contents, contentClassName }: CarouselSliderProps) => {
      ...
      <div className="slider-buttons flex space-x-3 justify-center mt-5">
        {contents.map((_, index) => (
          <span
            ...
            className={
              'block w-3 h-3 bg-zinc-200 rounded-full cursor-pointer ' + (currentSlide === index ? 'bg-zinc-500' : '')
            }
          ></span>
        ))}
      </div>
